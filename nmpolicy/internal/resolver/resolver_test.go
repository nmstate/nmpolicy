...
invalid-path-type:
  ...
  - pos: 7
    string: 10.244.0.1
`,
			err: `resolve error: eqfilter error: ` +
				`failed applying operation on the path: ` +
				`type missmatch: "[]interface {}" != "string" -> ` +
				`[map[ip:10.244.0.1 prefix-length:24] map[ip:169.254.1.0 prefix-length:16]] != 10.244.0.1`,
		}
		runTest(t, testToRun)
	})
}
